...
public record RiderAssignedRace(int RaceId, string Name, DateOnly StartDate, DateOnly EndDate, int StageCount);
...
public class RosterService
{
    ...
    public async Task<List<RiderAssignedRace>> GetAllAssignedRacesAsync()
    {
        ...
        await using var command = connection.CreateCommand();
        command.CommandText = """
            SELECT rc.id, rc.name, rc.start_date, rc.end_date,
        ...
        while (await reader.ReadAsync())
        {
            ...
            races.Add(new RiderAssignedRace(reader.GetInt32(0), reader.GetString(1), startDate, endDate, reader.GetInt32(4)));
        }
        return races;
    }
    ...
    public async Task<List<RiderAssignedRace>> GetRiderAssignedRacesAsync(int riderId)
    {
        ...
        await using var command = connection.CreateCommand();
        command.CommandText = """
            SELECT rc.id, rc.name, rc.start_date, rc.end_date, COUNT(s.id) AS stage_count
            FROM race rc
            ...
            GROUP BY rc.id, rc.name, rc.start_date, rc.end_date
            ORDER BY rc.start_date
            """;
        command.Parameters.AddWithValue("@riderId", riderId);
